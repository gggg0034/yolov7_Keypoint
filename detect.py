    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for det_index, det_data in enumerate(det[:,:18]):
                    xyxy = det_data[:4]
                    conf = det_data[4]
                    cls = det_data[5]
                    kpts = [det_data[6], det_data[7], det_data[9], det_data[10], det_data[12], det_data[13], det_data[15], det_data[16]]
                    if save_txt:  # Write to file
                        xyxy_tensor = torch.tensor(xyxy).to(device)  # Move tensor to device
                        gn_tensor = gn.to(device)  # Move tensor to device
                        xywh = (xyxy2xywh(xyxy_tensor.view(1, 4)) / gn_tensor).view(-1).tolist()  # normalized xywh

                        kps_tensor = torch.tensor(kpts).view(4, 2).to(device)  # Move tensor to device and reshape to 4x2
                        kps = kps_tensor.view(-1).tolist()  # Flatten to 8 values
                        line = (cls, *xywh, *kps, conf) if opt.save_conf else (cls, *xywh, *kps)  # label format
